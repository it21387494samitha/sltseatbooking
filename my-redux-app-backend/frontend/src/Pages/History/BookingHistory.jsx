...
const BookingHistory = () => {
  ...
  useEffect(() => {
    const fetchBookingHistory = async () => {
      try {
        ...
        const response = await axios.get('https://newslt123.azurewebsites.net/api/seats/history', {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`https://newslt123.azurewebsites.net/api/seats/history/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
